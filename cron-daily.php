...
function cron(){
	...
	$sth = $db->query('SELECT * FROM '._DB_PREFIX_.'photos WHERE (offer_id=0 OR offer_id IS NULL) and date<(CURDATE() - INTERVAL 1 DAY)');
	foreach($sth as $row){
		unlink(realpath(dirname(__FILE__)).'/upload/photos/'.$row['thumb']);
		unlink(realpath(dirname(__FILE__)).'/upload/photos/'.$row['url']);
	}
	$db->query('DELETE FROM '._DB_PREFIX_.'photos WHERE (offer_id=0 OR offer_id IS NULL) and date<(CURDATE() - INTERVAL 1 DAY)');
	...
	foreach($sth as $row){
		mails_queue_add('finish_promote',$row['email'],array('offer_name'=>$row['name'], 'offer_url'=>$row['id'].','.$row['name_url']),5);
	}
