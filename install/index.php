...
?>');
			...
			$sql = file_get_contents('festa.sql');
	...
	<form method="post" action="" id="form_install">
		<table>
			...
			<tr>
				...
				<td><input type="text" name="db_prefix" placeholder="<?php echo(lang('Prefix tables in the database')); ?>" value="<?php if(isset($_POST['db_prefix'])){echo($_POST['db_prefix']);}?>" title="<?php echo(lang('In cases where multiple sites use the one database you can determine the prefix table, otherwise leave it blank')); ?>"/></td>
			</tr>
			<tr>
				<td><?php echo(lang('Salt passwords in the system')); ?>:</td>
				<td><input type="text" name="pass_hash" placeholder="<?php echo(lang('Salt passwords in the system')); ?>" value="<?php if(isset($_POST['pass_hash'])){echo($_POST['pass_hash']);}?>" title="<?php echo(lang('Additional security user passwords - enter any string')); ?>" pattern="[a-z_]*"/></td>
			</tr>
		</table>
