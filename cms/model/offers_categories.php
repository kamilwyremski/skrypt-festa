...
if($user_cms->logged_in){
	...
	if(!_CMS_TEST_MODE_ and isset($_POST['action'])){
		if($_POST['action']=='add_category' and isset($_POST['name']) and $_POST['name']!=''){
			$sth = $db->prepare('INSERT INTO `'._DB_PREFIX_.'offers_categories`(`name_url`, `name`) VALUES (:name_url,:name)');
			...
			$sth = $db->prepare('UPDATE `'._DB_PREFIX_.'offers_categories` SET  name_url=:name_url, name=:name WHERE id=:id limit 1');
