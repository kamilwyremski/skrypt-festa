...
{if isset($offers)}
		...
		{foreach from=$pagination.page_url.sort item=item key=key}
			{if is_array($item)}
				{foreach from=$item item=item2 key=key2}
					<input type="hidden" name="{$key}[]" value="{$item2}">
				{/foreach}
			{else}
	...
	{foreach from=$offers item=item key=key}
		<div class="offers" itemscope itemtype="https://schema.org/Accommodation">
			<div class="row {if $item.promoted}promoted{/if}">
				...
				<div class="col-md-5 overflow_hidden">
					...
					<p>{if !empty($item.type)}<a href="{$links.offers}?search=&type%5B%5D={$item.type.name_url}" title="{'Offer type'|lang}: {$item.type.name}" class="text-success">{$item.type.name}</a>{/if}
						{if !empty($item.type) && !empty($item.state)} | {/if}
						{if !empty($item.state)}<a href="{$links.offers}?search=&state%5B%5D={$item.state.name_url}" title="{'State'|lang}: {$item.state.name}" class="text-success"><span itemprop="address">{$item.state.name}</span></a>{/if}
					</p>
				...
				<div class="col-md-2">
					{if !empty($item.category)}<div class="offers_price offers_price_top text-center"><p><strong><a href="{$links.offers}?search=&category%5B%5D={$item.category.name_url}" title="{'Category'|lang}: {$item.category.name}">{$item.category.name}</a></strong></p></div>{/if}
					{if isset($item.price_from)}<div class="offers_price offers_price_bottom text-center"><p>{'Prices from'|lang}:</p><p><strong>{$item.price_from} {$settings.currency}</strong></p></div>{/if}
				</div>
				{if $page=='my_offers'}
