...
if($settings['photo_add'] and isset($_FILES["file"]["type"])){
	...
	$old_name = substr(name_url($path_parts['filename']),0,200).'.'.$extension;

	if(file_exists('../upload/photos/'.$old_name)) {  
		$img = explode('.', $_FILES['file']['name']);
		$url = name_url($img[0]).'_'.time().'.'.$extension;
	}else{
		$url = $old_name;
	}
	if(!in_array($extension , array('jpg','jpeg', 'gif', 'png', 'JPG', 'JPEG', 'GIF', 'PNG'))){
		...
		if($settings['watermark_add'] and $settings['watermark']!=''){
			...
			$stamp_extension = substr(strrchr($settings['watermark'], "."), 1); 
